...
#Q_4_b
	"touch ~/index.txt"
	"echo '01' > ~/serial"
	"openssl "{'genrsa -out gened.key','req -subj "/C=GR/ST=ATTIKI/L=ATHENS/O=MY_MYSELF_I/CN=my.domain.netmode.ntua.gr" -new -key gened.key -keyform PEM -out requed.csr','ca -in requed.csr -out fin.crt','s_client -state -host sorch.netmode.ntua.gr -port 443','s_client -state -host sorch.netmode.ntua.gr -port 443 -cert fin.crt -key gened.key'}" 2>&1 | tee -a $outfile"
#	"openssl s_client -state -host sorch.netmode.ntua.gr -port 443 -cert fin.crt -key gened.key < lab4_ssl_command.txt 2>&1 | tee -a $outfile"
	####On the last we have to type "GET /netmg.php HTTP/1.0\n\n", since the server has no expect shell and input redirection seems not to function.
)
